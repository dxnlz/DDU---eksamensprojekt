...
cat "${DIR}/testdata/users.csv" | run_query "COPY users (id, username, birthday, country, registered, password) FROM STDIN DELIMITER ',' CSV HEADER;"
cat "${DIR}/testdata/reviews.csv" | run_query "COPY reviews (id, stars, title, description, author, created, product, last_updated) FROM STDIN DELIMITER ',' CSV HEADER;"
